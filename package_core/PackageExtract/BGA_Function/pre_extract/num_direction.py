...
def add_direction_field_to_yolox_nums(L3):
    ...
    for view in ("top", "bottom", "side", "detailed"):
        ...
        border_valid = False
        if isinstance(border, np.ndarray):
            if border.size > 0 and len(border) > 0:
                border_valid = True
        elif border and len(border) > 0:
            border_valid = True
        ...
        # 创建带方向信息的yolox_nums副本
        # 无论yolox_nums是什么类型，都将其转换为字典列表
        yolox_nums_with_direction = []
        ...
        for item in yolox_nums:
            if isinstance(item, dict):
                ...
                if 'location' not in new_item:
                    # 如果没有location字段，尝试从其他可能的字段获取
                    if 'bbox' in new_item:
                        new_item['location'] = new_item['bbox']
                    elif 'coordinates' in new_item:
                        new_item['location'] = new_item['coordinates']
                    else:
                        # 如果没有找到位置信息，使用整个字典作为位置（可能需要调整）
                        new_item['location'] = new_item
            else:
                # 如果不是字典，假设它是位置信息（列表或数组）
                if isinstance(item, np.ndarray):
                    location = item.tolist()
                else:
                    location = item
                new_item = {
                    'location': location,
            ...
            yolox_nums_with_direction.append(new_item)
        ...
        for yolox_num in yolox_nums_with_direction:
            ...
            if isinstance(loc, list) and len(loc) == 4:  # [x1, y1, x2, y2]格式
                obj_center_x = (loc[0] + loc[2]) / 2
                obj_center_y = (loc[1] + loc[3]) / 2
            elif isinstance(loc, dict):
                # 如果location是字典，尝试从中提取坐标
                if 'x1' in loc and 'y1' in loc and 'x2' in loc and 'y2' in loc:
                    obj_center_x = (loc['x1'] + loc['x2']) / 2
                    obj_center_y = (loc['y1'] + loc['y2']) / 2
                else:
                    # 无法提取坐标，使用默认值
                ...
                # 假设是其他格式，使用前两个值作为中心点
                obj_center_x = loc[0] if len(loc) > 0 else center_x
                obj_center_y = loc[1] if len(loc) > 1 else center_y
